...
1. `AWS_DEFAULT_REGION`: The region for created instances.
1. `AUTH_USER`: Username for administrative access to the broker itself.
1. `AUTH_PASS`: Password for administrative access to the broker itself.
1. `ENC_KEY`: Password for encrypting secrets internally.

> Note the AWS Environment Variables should be generated by following the instructions [here](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSGettingStartedGuide/AWSCredentials.html)
...
If you aren't sure what's required, see the respective service user guides:
* [RDS](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/CHAP_GettingStarted.html)
* [S3](https://docs.aws.amazon.com/AmazonS3/latest/gsg/GetStartedWithS3.html)

#### Secrets.yml

secrets.yml contains all of the secrets for the different resources. Each plan requires a database (not necessarily hosted on AWS).
...
In this case BROKER_NAME would be `aws` and it would contain many service names (one for `rds`, one for `s3`). Then SERVICE_NAME would be `aws-rds` for example.
