resource "aws_db_instance" "rds_database" {
  ...
  iops              =  var.rds_db_size >= 400 ? var.rds_db_iops : null
  ...
  parameter_group_name = var.rds_db_engine == "postgres" ? join(
    "",
    aws_db_parameter_group.recreatable_parameter_group_postgres.*.id,
    ) : join(
    "",
    aws_db_parameter_group.recreatable_parameter_group_mysql.*.id,
  )
