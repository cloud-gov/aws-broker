...
> **Status note (2026-08-13):** the **core decision holds and is reflected in
> `main`** — `aws-broker` runs no InSpec/Cinc, and validation lives in the overlay
> (`cg-oracle-database-19c-stig-overlay`), which today reaches a brokered instance
> via `cf ssh` (`runner/run-validation.sh`). The **JSON validation-contract
> interface (#530) is not yet built** — it is a proposed future interface, not
> current state. This ADR is Accepted for the separation-of-concerns decision; the
> contract detail is aspirational until #530 lands.
