...
> **Status note (2026-08-13):** This ADR is **Proposed, not yet reflected in `main`.**
> The initial Oracle work merged (#564) took a deliberately **minimal, imperative**
> approach — `services/rds/oracle_tls.go` for the SSL option group — and did **not**
> introduce the `RDSBaseline` engine-strategy interface, the
> `services/rds/baselines/oracle19c/*.yml` files, or a refactor of the existing
> MySQL/Postgres engines. No Oracle parameter-group hardening is on `main` yet.
> Whether to adopt this engine abstraction or instead add a minimal `oracle-se2`
> branch to the existing `parameter_group.go` is an **open decision**, tracked in
> [#568](https://github.com/cloud-gov/aws-broker/issues/568). This record preserves
> the rationale; it will move to **Accepted** only if/when the abstraction lands.
