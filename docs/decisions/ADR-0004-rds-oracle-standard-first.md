...
> **Status note (2026-08-13):** SE2 + License Included **is reflected in `main`**
> (`dbType: oracle-se2`, `licenseModel: license-included`, `encrypted: true`;
> TLS option group via `oracle_tls.go`; opt-in Multi-AZ plan `medium-oracle-se2-redundant`).
> The **auditing compensating control above depends on the `audit_trail` parameter,
> which is not yet configured on `main`** — the Oracle parameter-group hardening is
> pending (see [ADR-0003](ADR-0003-design-oracle-baseline-for-future-csb-portability.md),
> tracked in [#568](https://github.com/cloud-gov/aws-broker/issues/568)). The edition/
> license decision is Accepted; the audit compensating control is intended and lands
> with the parameter-group hardening.
