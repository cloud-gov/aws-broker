...
| [ADR-0003](ADR-0003-sql-hardening-platform-vs-customer-responsibility.md) | In-database SQL-statement STIG hardening (`hardening/sql/`) is a customer responsibility; the broker ships and validates the SQL but never runs it against tenant DBs. CI validation planned | Accepted |
