...
`cf create-service aws-rds oracle-19c-dev my-oracle` drives the standard RDS
async lifecycle (see [current-state-analysis.md](current-state-analysis.md) §2):

1. `rdsBroker.CreateInstance` validates options, fetches the `oracle-19c-dev`
...
3. A **River job** is enqueued; `CreateInstance` returns `in progress`. CF polls
   `LastOperation`.
4. The worker builds `CreateDBInstanceInput`: `Engine=oracle-ee`, pinned
   `EngineVersion`, `LicenseModel=bring-your-own-license`, `StorageEncrypted`,
...
Verified against a **moto** RDS control-plane mock: `create-db-instance` (oracle-ee
19c, encrypted, BYOL, private), `create-db-parameter-group` (`oracle-ee-19`), and
`create-option-group` (oracle-ee 19) are all accepted — control-flow signal, not
