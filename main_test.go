...
var modifyRDSMySQLWithVersionInstanceReq = []byte(
	`{
	"service_id":"db80ca29-2d1b-4fbc-aad3-d03c0bfa7593",
	"plan_id":"da91e15c-98c9-46a9-b114-02b8d28062c7",
	"parameters": {
...
var modifyRDSMySQLWithInvalidVersionInstanceReq = []byte(
	`{
	"service_id":"db80ca29-2d1b-4fbc-aad3-d03c0bfa7593",
	"plan_id":"da91e15c-98c9-46a9-b114-02b8d28062c7",
	"parameters": {
...
var modifyRDSMySQLWithInvalidPreviousVersionInstanceReq = []byte(
	`{
	"service_id":"db80ca29-2d1b-4fbc-aad3-d03c0bfa7593",
	"plan_id":"da91e15c-98c9-46a9-b114-02b8d28062c7",
	"parameters": {
...
func TestModifyRDSInstanceValidVersion(t *testing.T) {
	// We need to create an instance first before we can try to modify it.
	createURL := "/v2/service_instances/the_RDS_instance?accepts_incomplete=true"
	res, m := doRequest(nil, createURL, "PUT", true, bytes.NewBuffer(createRDSMySQLInstanceReq))
	...
	println("Current DB Version is " + i.DbVersion)

	// Check to make sure the instance has the original plan set on it.
	if i.PlanID != originalRDSMySQLPlanID {
		t.Error("The instance should have the plan provided with the create request.")
	}

	urlUnacceptsIncomplete := "/v2/service_instances/the_RDS_instance"
	resp, _ := doRequest(m, urlUnacceptsIncomplete, "PATCH", true, bytes.NewBuffer(modifyRDSMySQLWithVersionInstanceReq))
	...
	// Pull in Version and upgrade the instance.
	urlAcceptsIncomplete := "/v2/service_instances/the_RDS_instance?accepts_incomplete=true"
	resp, _ = doRequest(m, urlAcceptsIncomplete, "PATCH", true, bytes.NewBuffer(modifyRDSMySQLWithVersionInstanceReq))
	...
	// Check to make sure the version update was accepted.
	// Is it a valid JSON?
	validJSON(res.Body.Bytes(), urlAcceptsIncomplete, t)
	...
	if !strings.Contains(i.DbVersion, "8.0") {
		t.Error("The DB Version for the instance should be 8.0.x; it is still " + i.DbVersion)
	}
}

func TestModifyRDSInstanceInvalidVersion(t *testing.T) {
	// We need to create an instance first before we can try to modify it.
	createURL := "/v2/service_instances/the_RDS_instance?accepts_incomplete=true"
	res, m := doRequest(nil, createURL, "PUT", true, bytes.NewBuffer(createRDSMySQLInstanceReq))
	...
	urlUnacceptsIncomplete := "/v2/service_instances/the_RDS_instance"
	resp, _ := doRequest(m, urlUnacceptsIncomplete, "PATCH", true, bytes.NewBuffer(modifyRDSMySQLWithInvalidVersionInstanceReq))
	...
	// Pull in Version and upgrade the instance.
	urlAcceptsIncomplete := "/v2/service_instances/the_RDS_instance?accepts_incomplete=true"
	resp, _ = doRequest(m, urlAcceptsIncomplete, "PATCH", true, bytes.NewBuffer(modifyRDSMySQLWithInvalidVersionInstanceReq))
	...
	// Check to make sure the version update was rejected.
	// Is it a valid JSON?
	validJSON(res.Body.Bytes(), urlAcceptsIncomplete, t)

	// Does it contain "Invalid version specified: please provide a version number between"?
	if !strings.Contains(string(resp.Body.Bytes()), "Invalid version specified: please provide a version number between") {
		t.Error(urlAcceptsIncomplete, "should return a message that the version cannot be chosen")
	}

	// Reload the instance and check to see that the plan has not been modified.
	i = rds.RDSInstance{}
	brokerDB.Where("uuid = ?", "the_RDS_instance").First(&i)
	if !strings.Contains(i.DbVersion, "5.7") {
		t.Logf("The instance was modified: " + i.DbVersion + " != " + "5.7.x")
		t.Error("The instance was modified to have a new version when it should not have been.")
	}
}

func TestModifyRDSInstanceInvalidPreviousVersion(t *testing.T) {
	// We need to create an instance first before we can try to modify it.
	createURL := "/v2/service_instances/the_RDS_instance?accepts_incomplete=true"
	res, m := doRequest(nil, createURL, "PUT", true, bytes.NewBuffer(createRDSLatestMySQLInstanceReq))
	...
	urlUnacceptsIncomplete := "/v2/service_instances/the_RDS_instance"
	resp, _ := doRequest(m, urlUnacceptsIncomplete, "PATCH", true, bytes.NewBuffer(modifyRDSMySQLWithInvalidPreviousVersionInstanceReq))
	...
	// Pull in Version and upgrade the instance.
	urlAcceptsIncomplete := "/v2/service_instances/the_RDS_instance?accepts_incomplete=true"
	resp, _ = doRequest(m, urlAcceptsIncomplete, "PATCH", true, bytes.NewBuffer(modifyRDSMySQLWithInvalidPreviousVersionInstanceReq))
	...
	// Does it contain "Cannot revert to an older version of this database engine."?
	if !strings.Contains(string(resp.Body.Bytes()), "Cannot revert to an older version of this database engine.") {
		t.Error(urlAcceptsIncomplete, "should return a message that the version cannot be reverted to an older version")
	}
	...
	brokerDB.Where("uuid = ?", "the_RDS_instance").First(&i)
	if !strings.Contains(i.DbVersion, "8.0") {
		t.Logf("The instance was modified: " + i.DbVersion + " != " + "8.0.x")
		t.Error("The instance was modified to have a new version when it should not have been.")
	}
}
