...
// the expected STIG-relevant content. These run offline; they
// are development signal for the values, not compliance evidence.
...
func TestOracleOptionsBaselineHasNoAttackSurfaceOptions(t *testing.T) {
	...
	// The baseline must not enable attack-surface options.
...
// provision path: an ECDSA-only cipher on the RSA CA is rejected before
