...
func TestGetCredentials(t *testing.T) {
	testCases := map[string]struct {
		...
		"oracle-se2": {
			...
			// + DN match + GovCloud CA bundle. Port is the SSL port, not 1521.
			// No hardcoded cert DN: the driver verifies identity via the CA bundle +
			// ssl_server_dn_match (pg/mysql omit it; the RDS server DN is Amazon-owned).
			expectedCreds: map[string]string{
...
// guard: the RDSBaseline refactor must
...
// TestOracleBindingDoesNotLeakAdminMarkers guards the master-credential concern: the
